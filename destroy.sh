...
for OBJECT in $(aws s3api list-object-versions --bucket $BUCKET | jq -r '"\(.DeleteMarkers[].Key),\(.DeleteMarkers[].VersionId)"' 2>/dev/null); do
...
for OBJECT in $(aws s3api list-object-versions --bucket $BUCKET | jq -r '"\(.Versions[].Key),\(.Versions[].VersionId)"' 2>/dev/null); do
  KEY=$(echo $OBJECT | cut -d ',' -f1)
  VERSIONID=$(echo $OBJECT | cut -d ',' -f2)
  aws s3api delete-object --bucket $BUCKET --version-id $VERSIONID --key $KEY 2>/dev/null >/dev/null
done
